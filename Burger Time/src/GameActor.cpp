	...
	Component(owner), m_lives(3), m_score(0)
{
	...
	m_spawnPosition = owner->GetWorldPosition();
...
void dae::GameActor::Update()
{
	...
	{
		const auto overlaps = m_collisionComponent->GetOverlapsByLayer(CollisionLayer::Enemy, false);
		if (!overlaps.empty())
		{
			OnDeath();
		}
